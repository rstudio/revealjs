...
version <- Sys.getenv(
  "REVEALJS_VERSION",
  xfun::github_releases("hakimel/reveal.js", pattern = "([0-9.]+)")[1]
)
url <- sprintf(
  "https://github.com/hakimel/reveal.js/archive/refs/tags/%s.zip",
  version
)
...
fs::dir_copy(
  reveal_folder,
  fs::path("inst", fs::path_file(reveal_folder)),
  overwrite = TRUE
)
...
ignore <- fs::dir_ls(
  current,
  regexp = ".*(dist|plugin|LICENSE|README.md).*",
  invert = TRUE,
  all = TRUE
)
...
version <- if (fs::file_exists(version_file)) {
...
url <- sprintf(
  "https://github.com/denehyg/reveal.js-menu/archive/refs/tags/%s.zip",
  latest
)
xfun::download_file(url)
fs::dir_ls()
...
fs::file_copy(
  fs::path(new_plugin, to_keep),
  fs::path(plugin_folder, to_keep),
  overwrite = TRUE
)
...
url <- sprintf(
  "https://github.com/rajgoel/reveal.js-plugins/archive/refs/tags/%s.zip",
  latest
)
xfun::download_file(url)
fs::dir_ls()
